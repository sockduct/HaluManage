...
def re2_approach(system_prompt, initial_query, client, model, n=1, request_id: str = None):
    ...
    try:
        ...
        # Log provider call
        if hasattr(halumanage, 'conversation_logger') and halumanage.conversation_logger and request_id:
            response_dict = response.model_dump() if hasattr(response, 'model_dump') else response
            halumanage.conversation_logger.log_provider_call(request_id, provider_request, response_dict)
