# halumanage Proxy Plugin

A sophisticated load balancing and failover plugin for halumanage that distributes requests across multiple LLM providers.
...
- 🔌 **Universal Compatibility**: Works with any halumanage approach or plugin
...
```bash
# Install halumanage via pip
pip install halumanage
...
Create `~/.halumanage/proxy_config.yaml`:
...
```bash
export halumanage_LOG_LEVEL=DEBUG
...
Part of halumanage - see main project license.