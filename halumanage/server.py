...
from halumanage.mcts import chat_with_mcts
from halumanage.bon import best_of_n_sampling
from halumanage.moa import mixture_of_agents
from halumanage.rto import round_trip_optimization
from halumanage.self_consistency import advanced_self_consistency_approach
from halumanage.pvg import inference_time_pv_game
from halumanage.z3_solver import Z3SymPySolverSystem
from halumanage.rstar import RStar
from halumanage.cot_reflection import cot_reflection
from halumanage.plansearch import plansearch
from halumanage.leap import leap
from halumanage.reread import re2_approach
from halumanage.cepo.cepo import cepo, CepoConfig, init_cepo_config
from halumanage.mars import multi_agent_reasoning_system
from halumanage.batching import RequestBatcher, BatchingError
from halumanage.conversation_logger import ConversationLogger
import halumanage.conversation_logger
...
def get_config():
    ...
    if os.environ.get("halumanage_API_KEY"):
        # Use local inference engine
        from halumanage.inference import create_inference_client
        API_KEY = os.environ.get("halumanage_API_KEY")
...
def load_plugins():
   ...
   import halumanage
   package_plugin_dir = os.path.join(os.path.dirname(halumanage.__file__), 'plugins')
   
   # Get local project plugins directory
   current_dir = os.getcwd() if server_config.get("plugins_dir", "") == "" else server_config["plugins_dir"]
   local_plugin_dir = os.path.join(current_dir, 'halumanage', 'plugins')
...
def get_config_path():
    ...
    import halumanage
    package_config_dir = os.path.join(os.path.dirname(halumanage.__file__), 'cepo', 'configs')
    ...
    local_config_dir = os.path.join(current_dir, 'halumanage', 'cepo', 'configs')
...
def parse_conversation(messages):
    ...
    for message in messages:
        ...
        if role == 'system':
            system_prompt, halumanage_approach = extract_halumanage_approach(text_content)
        elif role == 'user':
            if not halumanage_approach:
                text_content, halumanage_approach = extract_halumanage_approach(text_content)
            conversation.append(f"User: {text_content}")
        elif role == 'assistant':
            conversation.append(f"Assistant: {text_content}")
    
    initial_query = "\n".join(conversation)
    return system_prompt, initial_query, halumanage_approach
...
def extract_halumanage_approach(content):
    match = re.search(r'<halumanage_approach>(.*?)</halumanage_approach>', content)
    if match:
        approach = match.group(1)
        content = re.sub(r'<halumanage_approach>.*?</halumanage_approach>', '', content).strip()
...
@app.before_request
def check_api_key():
    if server_config['halumanage_api_key']:
        ...
        if not secrets.compare_digest(client_key, server_config['halumanage_api_key']):
...
def proxy():
    ...
    halumanage_approach = data.get('halumanage_approach', server_config['approach'])
    logger.debug(data)
    ...
    system_prompt, initial_query, message_halumanage_approach = parse_conversation(messages)

    if message_halumanage_approach:
        halumanage_approach = message_halumanage_approach

    if halumanage_approach != "auto":
        model = f"{halumanage_approach}-{model}"
        ...
        if request_batcher is not None:
            try:
                ...
                batch_request_data = {
                    ...
                    'n': n,
                    'stream': stream,
                    'halumanage_approach': halumanage_approach
                }
...
def parse_args():
    ...
    try:
        from halumanage import __version__ as package_version
    except ImportError:
    ...
    args_env = [
        ("--halumanage-api-key", "halumanage_API_KEY", str, "", "Optional API key for client authentication to halumanage"),
        ("--approach", "halumanage_APPROACH", str, "auto", "Inference approach to use", known_approaches + list(plugin_approaches.keys())),
        ("--mcts-simulations", "halumanage_SIMULATIONS", int, 2, "Number of MCTS simulations"),
        ("--mcts-exploration", "halumanage_EXPLORATION", float, 0.2, "Exploration weight for MCTS"),
        ("--mcts-depth", "halumanage_DEPTH", int, 1, "Simulation depth for MCTS"),
        ("--model", "halumanage_MODEL", str, "gpt-4o-mini", "OpenAI model to use"),
        ("--rstar-max-depth", "halumanage_RSTAR_MAX_DEPTH", int, 3, "Maximum depth for rStar algorithm"),
        ("--rstar-num-rollouts", "halumanage_RSTAR_NUM_ROLLOUTS", int, 5, "Number of rollouts for rStar algorithm"),
        ("--rstar-c", "halumanage_RSTAR_C", float, 1.4, "Exploration constant for rStar algorithm"),
        ("--n", "halumanage_N", int, 1, "Number of final responses to be returned"),
        ("--return-full-response", "halumanage_RETURN_FULL_RESPONSE", bool, False, "Return the full response including the CoT with <thinking> tags"),
        ("--port", "halumanage_PORT", int, 8000, "Specify the port to run the proxy"),
        ("--log", "halumanage_LOG", str, "info", "Specify the logging level", list(logging_levels.keys())),
        ("--launch-gui", "halumanage_LAUNCH_GUI", bool, False, "Launch a Gradio chat interface"),
        ("--plugins-dir", "halumanage_PLUGINS_DIR", str, "", "Path to the plugins directory"),
        ("--log-conversations", "halumanage_LOG_CONVERSATIONS", bool, False, "Enable conversation logging with full metadata"),
        ("--conversation-log-dir", "halumanage_CONVERSATION_LOG_DIR", str, str(Path.home() / ".halumanage" / "conversations"), "Directory to save conversation logs"),
    ]
    ...
    best_of_n_default = int(os.environ.get("halumanage_BEST_OF_N", 3))
    ...
    base_url_default = os.environ.get("halumanage_BASE_URL", "")
    parser.add_argument("--base-url", "--base_url", dest="base_url", type=str, default=base_url_default,
                        help="Base url for OpenAI compatible endpoint")

    # SSL configuration arguments
    ssl_verify_default = os.environ.get("halumanage_SSL_VERIFY", "true").lower() in ("true", "1", "yes")
    ...
    ssl_cert_path_default = os.environ.get("halumanage_SSL_CERT_PATH", "")
    ...
    batch_mode_default = os.environ.get("halumanage_BATCH_MODE", "false").lower() == "true"
    batch_size_default = int(os.environ.get("halumanage_BATCH_SIZE", 4))
    batch_wait_ms_default = int(os.environ.get("halumanage_BATCH_WAIT_MS", 50))
...
def main():
    ...
    if server_config.get('batch_mode', False):
        ...
        def process_batch_requests(batch_requests):
            ...
            """
            import time
            from halumanage.batching import BatchingError
    ...
    halumanage.conversation_logger.set_global_logger(conversation_logger)
    ...
    if server_config_clean['halumanage_api_key']:
        server_config_clean['halumanage_api_key'] = '[REDACTED]'
    ...
    if server_config.get('launch_gui'):
        try:
            ...
            def chat_with_halumanage(message, history):
                import httpx
            ...
            demo = gr.ChatInterface(
                chat_with_halumanage,
                title="halumanage Chat Interface",
                description=f"Connected to halumanage proxy at {base_url}"
            )
