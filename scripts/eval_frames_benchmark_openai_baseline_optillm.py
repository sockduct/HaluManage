...
# To call the OpenAI API directly, we initialize the client without a `base_url`.
# The client will automatically use the default OpenAI endpoint.
...
def generate_llm_prompt(prompt: str, wiki_links: List[str]) -> str:
    # Use Navie prompt without any RAG content for baseline evaluation
    return prompt

def get_llm_response(prompt: str, model: str) -> str:
    print(f"Invoking OpenAI API with model: {model}")
...
def main(model: str):
    ...
    for item in tqdm(dataset, desc="Processing samples"):
        ...
        #tqdm.write(f"llm_response: {llm_response}")
