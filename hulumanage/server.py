...
from hulumanage.mcts import chat_with_mcts
from hulumanage.bon import best_of_n_sampling
from hulumanage.moa import mixture_of_agents
from hulumanage.rto import round_trip_optimization
from hulumanage.self_consistency import advanced_self_consistency_approach
from hulumanage.pvg import inference_time_pv_game
from hulumanage.z3_solver import Z3SymPySolverSystem
from hulumanage.rstar import RStar
from hulumanage.cot_reflection import cot_reflection
from hulumanage.plansearch import plansearch
from hulumanage.leap import leap
from hulumanage.reread import re2_approach
from hulumanage.cepo.cepo import cepo, CepoConfig, init_cepo_config
from hulumanage.mars import multi_agent_reasoning_system
from hulumanage.batching import RequestBatcher, BatchingError
from hulumanage.conversation_logger import ConversationLogger
import hulumanage.conversation_logger
...
def get_config():
    ...
    if os.environ.get("hulumanage_API_KEY"):
        # Use local inference engine
        from hulumanage.inference import create_inference_client
        API_KEY = os.environ.get("hulumanage_API_KEY")
    ...
    # Cerebras, OpenAI, Azure, or LiteLLM API configuration
        ...
        from hulumanage.litellm_wrapper import LiteLLMWrapper
...
def load_plugins():
   ...
   import hulumanage
   package_plugin_dir = os.path.join(os.path.dirname(hulumanage.__file__), 'plugins')
   
   # Get local project plugins directory
   current_dir = os.getcwd() if server_config.get("plugins_dir", "") == "" else server_config["plugins_dir"]
   local_plugin_dir = os.path.join(current_dir, 'hulumanage', 'plugins')
...
def get_config_path():
    ...
    import hulumanage
    package_config_dir = os.path.join(os.path.dirname(hulumanage.__file__), 'cepo', 'configs')
    ...
    local_config_dir = os.path.join(current_dir, 'hulumanage', 'cepo', 'configs')
...
def parse_conversation(messages):
    ...
    for message in messages:
        ...
        if role == 'system':
            system_prompt, hulumanage_approach = extract_hulumanage_approach(text_content)
        elif role == 'user':
            if not hulumanage_approach:
                text_content, hulumanage_approach = extract_hulumanage_approach(text_content)
            conversation.append(f"User: {text_content}")
        elif role == 'assistant':
            conversation.append(f"Assistant: {text_content}")
    
    initial_query = "\n".join(conversation)
    return system_prompt, initial_query, hulumanage_approach
...
def extract_hulumanage_approach(content):
    match = re.search(r'<hulumanage_approach>(.*?)</hulumanage_approach>', content)
    if match:
        approach = match.group(1)
        content = re.sub(r'<hulumanage_approach>.*?</hulumanage_approach>', '', content).strip()
...
@app.before_request
def check_api_key():
    if server_config['hulumanage_api_key']:
        ...
        if not secrets.compare_digest(client_key, server_config['hulumanage_api_key']):
...
def proxy():
    ...
    hulumanage_approach = data.get('hulumanage_approach', server_config['approach'])
    logger.debug(data)
    ...
    system_prompt, initial_query, message_hulumanage_approach = parse_conversation(messages)

    if message_hulumanage_approach:
        hulumanage_approach = message_hulumanage_approach

    if hulumanage_approach != "auto":
        model = f"{hulumanage_approach}-{model}"
        ...
        if request_batcher is not None:
            try:
                ...
                batch_request_data = {
                    ...
                    'n': n,
                    'stream': stream,
                    'hulumanage_approach': hulumanage_approach
                }
...
def parse_args():
    ...
    try:
        from hulumanage import __version__ as package_version
    except ImportError:
    ...
    args_env = [
        ("--hulumanage-api-key", "hulumanage_API_KEY", str, "", "Optional API key for client authentication to hulumanage"),
        ("--approach", "hulumanage_APPROACH", str, "auto", "Inference approach to use", known_approaches + list(plugin_approaches.keys())),
        ("--mcts-simulations", "hulumanage_SIMULATIONS", int, 2, "Number of MCTS simulations"),
        ("--mcts-exploration", "hulumanage_EXPLORATION", float, 0.2, "Exploration weight for MCTS"),
        ("--mcts-depth", "hulumanage_DEPTH", int, 1, "Simulation depth for MCTS"),
        ("--model", "hulumanage_MODEL", str, "gpt-4o-mini", "OpenAI model to use"),
        ("--rstar-max-depth", "hulumanage_RSTAR_MAX_DEPTH", int, 3, "Maximum depth for rStar algorithm"),
        ("--rstar-num-rollouts", "hulumanage_RSTAR_NUM_ROLLOUTS", int, 5, "Number of rollouts for rStar algorithm"),
        ("--rstar-c", "hulumanage_RSTAR_C", float, 1.4, "Exploration constant for rStar algorithm"),
        ("--n", "hulumanage_N", int, 1, "Number of final responses to be returned"),
        ("--return-full-response", "hulumanage_RETURN_FULL_RESPONSE", bool, False, "Return the full response including the CoT with <thinking> tags"),
        ("--port", "hulumanage_PORT", int, 8000, "Specify the port to run the proxy"),
        ("--log", "hulumanage_LOG", str, "info", "Specify the logging level", list(logging_levels.keys())),
        ("--launch-gui", "hulumanage_LAUNCH_GUI", bool, False, "Launch a Gradio chat interface"),
        ("--plugins-dir", "hulumanage_PLUGINS_DIR", str, "", "Path to the plugins directory"),
        ("--log-conversations", "hulumanage_LOG_CONVERSATIONS", bool, False, "Enable conversation logging with full metadata"),
        ("--conversation-log-dir", "hulumanage_CONVERSATION_LOG_DIR", str, str(Path.home() / ".hulumanage" / "conversations"), "Directory to save conversation logs"),
    ]
    ...
    best_of_n_default = int(os.environ.get("hulumanage_BEST_OF_N", 3))
    ...
    base_url_default = os.environ.get("hulumanage_BASE_URL", "")
    parser.add_argument("--base-url", "--base_url", dest="base_url", type=str, default=base_url_default,
                        help="Base url for OpenAI compatible endpoint")

    # SSL configuration arguments
    ssl_verify_default = os.environ.get("hulumanage_SSL_VERIFY", "true").lower() in ("true", "1", "yes")
    ...
    ssl_cert_path_default = os.environ.get("hulumanage_SSL_CERT_PATH", "")
    ...
    batch_mode_default = os.environ.get("hulumanage_BATCH_MODE", "false").lower() == "true"
    batch_size_default = int(os.environ.get("hulumanage_BATCH_SIZE", 4))
    batch_wait_ms_default = int(os.environ.get("hulumanage_BATCH_WAIT_MS", 50))
...
def main():
    ...
    if server_config.get('batch_mode', False):
        ...
        def process_batch_requests(batch_requests):
            ...
            """
            import time
            from hulumanage.batching import BatchingError
    ...
    hulumanage.conversation_logger.set_global_logger(conversation_logger)
    ...
    if server_config_clean['hulumanage_api_key']:
        server_config_clean['hulumanage_api_key'] = '[REDACTED]'
    ...
    if server_config.get('launch_gui'):
        try:
            ...
            def chat_with_hulumanage(message, history):
                import httpx
            ...
            demo = gr.ChatInterface(
                chat_with_hulumanage,
                title="hulumanage Chat Interface",
                description=f"Connected to hulumanage proxy at {base_url}"
            )
