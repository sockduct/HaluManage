#!/usr/bin/env python3
"""
Request Batching Module for hulumanage
...
class RequestBatcher:
    """
    Automatic request batching for hulumanage
    ...
    def _get_request_key(self, request_data: Dict[str, Any]) -> str:
        ...
        approach = request_data.get('hulumanage_approach', 'none')
    ...
    def _validate_batch_compatibility(self, requests: List[BatchRequest]) -> None:
        ...
        if len(approaches) > 1:
            raise BatchingError(f"Cannot batch different hulumanage approaches: {approaches}")
    ...
    def add_request(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            batch_request = BatchRequest(
                ...
                timestamp=time.time(),
                model=request_data.get('model', 'default'),
                approach=request_data.get('hulumanage_approach')
            )
