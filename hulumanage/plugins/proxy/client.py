...
class ProxyClient:
    ...
    class _Completions:
        ...
        def _filter_kwargs(self, kwargs: dict) -> dict:
            """Filter out hulumanage-specific parameters that shouldn't be sent to providers"""
            hulumanage_params = {
                'hulumanage_approach', 'proxy_wrap', 'wrapped_approach', 'wrap',
                'mcts_simulations', 'mcts_exploration', 'mcts_depth',
                'best_of_n', 'rstar_max_depth', 'rstar_num_rollouts', 'rstar_c'
            }
            return {k: v for k, v in kwargs.items() if k not in hulumanage_params}
        ...
        def create(self, **kwargs):
            ...
            try:
                ...
                while healthy_providers:
                    ...
                    try:
                        # Map model name if needed and filter out hulumanage-specific parameters
