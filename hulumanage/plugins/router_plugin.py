...
hulumanage_MODEL_NAME = "codelion/hulumanage-modernbert-large"
...
def load_hulumanage_model():
    ...
    safetensors_path = hf_hub_download(repo_id=hulumanage_MODEL_NAME, filename="model.safetensors")
    # Load the state dict from the safetensors file
    load_model(model, safetensors_path)

    tokenizer = AutoTokenizer.from_pretrained(hulumanage_MODEL_NAME)
...
def run(system_prompt, initial_query, client, model, **kwargs):
    try:
        ...
        router_model, tokenizer, device = load_hulumanage_model()
